...
export const completeContract = async (contractId: string) => {
  try {
    const response = await Axios.put(
      `/api/client/contract/complete/${contractId}`
    );
    console.log("Contract completed and funds released", response.data);
    return response.data;
  } catch (error: any) {
    throw error.response?.data || "Failed to complete contract";
  }
};
